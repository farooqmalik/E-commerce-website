...
if (isset($_POST)& !empty($_POST)) {
	if ($_POST['agree']==true) {
	$countary= filter_var($_POST['countary'],FILTER_SANITIZE_STRING);
	...
	//$r=mysqli_fetch_assoc($connection,$res);
	$count=mysqli_num_rows($res);
	if ($count==1) {
		//update data in usermeta table
		$usql="UPDATE usersmeta SET countary='$countary',firstname='$fname',lastname='$lname',company='$company',address1='$address1',address2='$address2',city='$city',state='$state',zip='$zip',mobile='$phone' WHERE uid= '$uid'";
		...
		if ($ures) {
			...
			$orditemres=mysqli_query($connection, $orditmsql) or die (mysqli_error($connection));
			// if ($orditemres) {
			// 	echo "OrderItems Inserted redirect to my account page.<br>";
			// }
	...
	echo	$isql="INSERT into usersmeta (countary,firstname,lastname,company,address1,address2,city,zip,state,mobile,uid) VALUES ('$countary','$fname','$lname','comapny','$address1','$address2','$city','$zip','$state','$phone','$uid')";
				...
				<div class="row">
					<div class="col-md-6 col-md-offset-3">
						<div class="billing-details">
							...
							<select name="countary" value="" class="form-control">
							...
							<div class="row">
								...
								echo $r['firstname']; } elseif(isset($fname)) {
									echo $fname;
								}?>" type="text" required>
								</div>
								<div class="col-md-6">
								...
								echo $r['lastname']; } elseif(isset($lname)) {
									echo $lname;
								}?>" type="text" required>
								</div>
							</div>
							...
							<input name="company" class="form-control" placeholder="" value="<?php if (!empty($r['company'])) {
								echo $r['company']; } elseif(isset($company)) {
									echo $company;
								}?>" type="text" required>
							<div class="clearfix space20"></div>
							<label>Address </label>
							<input name="address1" class="form-control" placeholder="Street address" value="<?php if (!empty($r['address1'])) {
								echo $r['address1']; } elseif(isset($address1)) {
									echo $address1;
								} ?>" type="text" required>
							<div class="clearfix space20"></div>
							<input name="address2" class="form-control"
								placeholder="Apartment, suite, unit etc. (optional)" value="<?php if (!empty($r['address2'])) {
								echo $r['address2']; } elseif(isset($address2)) {
									echo $address2;
								}?>" type="text" required>
							<div class="clearfix space20"></div>
							<div class="row">
								...
								echo $r['state']; }elseif(isset($state)) {
									echo $state;
							...
							<input name="phone" class="form-control" id="billing_phone" placeholder="" value="<?php if (!empty($r['mobile'])) {
								echo $r['mobile']; } elseif(isset($phone)) {
									echo $phone;
								} ?>" type="text" required>
